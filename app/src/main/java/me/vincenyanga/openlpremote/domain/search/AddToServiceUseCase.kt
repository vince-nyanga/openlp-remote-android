...
class AddToServiceUseCase(private val openLPApi: OpenLPApi): UseCase<AddPluginParams,Boolean>() {

    override fun execute(parameters: AddPluginParams): Boolean {
        val request = BaseRequest(parameters.data)
        val response = openLPApi.addToService(parameters.plugin, Gson().toJson(request)).execute()
